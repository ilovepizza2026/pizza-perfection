...
### ⚡ Quick unblock for Claire (30 seconds)

The murmur OAuth app (`Iv23liK7cN4LHJJrxDDB`) supports GitHub Device Flow. To complete Phase 2:

1. Go to **https://github.com/login/device** in your browser (while logged in as `clairerosenfeld`)
2. Enter the code that can be freshly generated by running:
   ```bash
   curl -s -X POST "https://github.com/login/device/code" \
     -H "Accept: application/json" \
     -d "client_id=Iv23liK7cN4LHJJrxDDB&scope=read:user"
   ```
3. Click **Authorize**
4. Re-run this murmur agent — it will detect the token and proceed to Phase 2

Device codes expire in 15 minutes each, so generate one fresh when you're ready.
...
**Blocker:** Device Flow requires `clairerosenfeld` to open `https://github.com/login/device` and enter the code while logged in to GitHub in their browser. The VM cannot automate this step — there are no stored GitHub browser credentials (only API tokens).

**Device Flow codes generated and polled in this audit (all expired without activation):**

| Code | device_code (prefix) | Generated | Polling result |
|---|---|---|---|
| `02DC-39B2` | `d7d9bb38...` | Session 1 | Expired unactivated |
| `3C34-8F44` | (unknown) | Session 1 | `{"error":"Not Found"}` when polled |
| `BEBB-2851` | `cc4f3c24...` | Session 2 (retry) | Expired unactivated after notify |
| `49D3-E4DD` | `2a25d456...` | Session 2 (retry) | Polled 3 min (36 attempts), not activated |
| `1206-2128` | `64de2735...` | Session 2 (retry) | Polled 2 min (24 attempts), not activated |

Each code was accompanied by a `murmur notify` to task `github_app/ilovepizza2026/w/pizza/github_oauth/clairerosenfeld/dashboard-audit`. No activation occurred during the ~2–3 min polling windows available. The Device Flow mechanism is confirmed working end-to-end — only the human activation step is missing.
...
### Phase 2 audit scope (planned — pending auth)

Once auth is established, the following tests will be executed:

**Desktop screenshots (1440 × 900px) — all major routes:**
- `/` — main task list / dashboard home
- `/settings` — workspace/org settings
- `/user-settings` — user account settings
- `/github_app/ilovepizza2026/task/<id>` — agent task detail page
- Any sub-routes discovered via SPA navigation

**Responsive screenshots (375px mobile, 768px tablet):**
- `/` — home
- A task detail page

**Functional checks:**
- Console errors and warnings (JS exceptions logged by Playwright)
- Network failures (4xx, 5xx on API calls or static assets)
- Empty states vs populated states
- Loading / spinner states
- Accessible role/aria attributes (tab order, button labels)
- Visual drift vs `docs.murmur.dev` screenshots (compare UI vs docs)

**gRPC-Web call audit (authenticated):**
- Which methods are called on page load?
- Do all calls succeed (`grpc-status: 0`)?
- Any unexpected errors?
- API latency distribution

All artifacts will be committed to `docs/audit/dashboard-audit/screenshots/` (named `12-` through `30-` to follow existing convention).
